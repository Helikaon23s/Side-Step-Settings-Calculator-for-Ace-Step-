...
def generate_smart_plan():
    print("="*85)
    print("ACE-STEP COMMAND CENTER: SINGLE-STEP GRADIENT MATRIX ENGINE V2.40")
    print("="*85)
    ...
    # =========================================================================
    # DYNAMIC TAIL CRUMB FILTER ENGINE
    # =========================================================================
    files = []
    total_duration = 0.0
    dropped_tails = 0
    ...
    # =========================================================================
    # AUTOMATED DATASET CHUNK CHARACTERIZATION ENGINE
    # =========================================================================
    ...
    is_vintage_analog = (mean_centroid > 2850.0) and (c_std < 0.28)
    rank, alpha = 32, 32
    rank_strategy = "Enforced 1:1 Matrix Unity (Standard Continuity Configuration)"
    rank_reason = "Balanced 1:1 scaling matrix optimized for cohesive single-artist modern digital production layouts."
    
    if num_singers >= 3:
        rank, alpha = 64, 64
        rank_strategy = "Multi-Concept Matrix (High Capacity)"
        rank_reason = f"Enforced high capacity to accommodate {num_singers} distinct vocal concepts without bleeding."
    elif is_vintage_analog:
        rank, alpha = 64, 64
        rank_strategy = "Enforced 1:1 Matrix Unity (Vintage Analog Configuration)"
        rank_reason = "High-capacity network dimensioning triggered by vintage spectral signature to capture raw analog warmth."
    ...
    # =========================================================================
    # QUALITY & ACCURACY ADAPTATION GRADIENTS
    # =========================================================================
    if quality_score >= 75.0:
        lr_gradient = 1.0 + ((quality_score - 75.0) / 25.0) * 0.15
        exponent_gradient = ((quality_score - 75.0) / 25.0) * -0.05
        epoch_gradient = 0
    else:
        lr_gradient = 1.0 - ((75.0 - quality_score) / 40.0) * 0.50
        exponent_gradient = ((75.0 - quality_score) / 40.0) * 0.25
        epoch_gradient = -int(((75.0 - quality_score) / 40.0) * 12)
    ...
    vintage_boost = 35.0 if is_vintage_analog else 0.0
    ...
    # Moved variable order constraint here to avoid NameError crash
    ...
    if mode == 3 and not is_lokr and rank != alpha and rank > 0:
        scaling_ratio = alpha / rank
        if scaling_ratio > 1.0: base_lr = base_lr / (scaling_ratio * 0.85)
        elif scaling_ratio < 1.0: base_lr = base_lr * (1.0 / scaling_ratio)
    ...
    # =========================================================================
    # HARDWARE VRAM TO TOPOLOGY BATCH/PRECISION SCALING ARCHITECTURE
    # =========================================================================
